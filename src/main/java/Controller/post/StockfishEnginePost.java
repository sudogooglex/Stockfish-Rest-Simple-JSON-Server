 ...
 */
@Data
public class StockfishEnginePost {

    private Process engineProcess;
    private BufferedReader processReader;
    private OutputStreamWriter processWriter;
// Server Input and Output
    private OutputBeanPost outputBean;
    private InputBeanPost inputBean;

    public StockfishEnginePost(InputBeanPost ib) {
        inputBean = ib;
        outputBean = new OutputBeanPost(ib.getForsythEdwardsNotation());
    }
     ...
     */
    public void init() throws IOException, InterruptedException, StockFishException {
...
// 2. Update Configuration
        ...
        // TODO : use command 'eval' to get statistics.
     ...
     * @throws Controller.main.StockFishException
     */
    public void updateNextBestMoove() throws IOException, InterruptedException, StockFishException {
// 4. Get the best move for a position with a given think time
        long startTime = System.currentTimeMillis();
     ...
     * @throws Controller.main.StockFishException
     */
    public void updateBoardState() throws IOException, InterruptedException, StockFishException {
        long startTime = System.currentTimeMillis();
// 5. Get all the legal moves from a given position // TODO : delete this
//        System.out.println("3. Legal moves : " + getLegalMoves());

// 6. Draw board from a given position
        outputBean.setBoardState(drawBoard(outputBean.getForsythEdwardsNotation())); // TODO : remove draw
        System.out.println("3. BoardState : \n"
                + outputBean.getBoardState() + "\n");
...
// 7. Get the evaluation score of current position
        long startTime = System.currentTimeMillis();
...
//        sendCommand("go movetime " + inputBean.getMovetime());
        ...
        nextBestMove = tSplit1[tSplitPos1];

// 2. A Next best moove is found
        String splitter2 = " ";
        if (nextBestMove.contains(splitter2)) {
            String[] tSplit2 = nextBestMove.split(splitter2);
            int tSplit2Length = tSplit2.length;
            if (tSplit2Length >= tSplit2Length) {
                nextBestMove = tSplit2[tSplitPos2] + " (" + nextBestMove.replace("\n", "") + ")";
            }
        }
...
//        return getOutput(0).split("Legal moves: ")[1];
     ...
     * @return
     */
    private String drawBoard(String fen) throws IOException, InterruptedException, StockFishException {
        String res;
        sendCommand("d");
